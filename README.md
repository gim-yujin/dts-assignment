...
- DB: PostgreSQL 16.13
- Build: Gradle 9.4.1
- Test: JUnit Jupiter 6.0.3, AssertJ 3.27.7
...
```bash
# 1. DB·유저 생성 (PostgreSQL 슈퍼유저로 실행)
...
| 테스트 클래스 | 범위 | 주요 검증 |
|---|---|---|
| `DtsAssignmentApplicationTests` | 통합 | 애플리케이션 컨텍스트 로드, 핵심 빈 등록 |
| `UserRepositoryTest` | 레포지토리 | username 조회, 존재하지 않는 사용자 처리, ID 자동 생성 |
| `AuthControllerTest` | 컨트롤러 | 로그인 화면 렌더링, 미인증 리다이렉트, 인증 후 홈 화면, 로그아웃 화면 |
